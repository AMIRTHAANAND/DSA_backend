...
router.get('/logins', requireRole([Role.ADMIN, Role.SUPER_ADMIN]), async (_req, res) => {
...
router.get('/pending', requireRole([Role.ADMIN, Role.SUPER_ADMIN]), async (_req, res) => {
  const users = await prisma.user.findMany({ where: { isApproved: false } });
  res.json(users);
});

router.post('/approve/:userId', requireRole([Role.ADMIN, Role.SUPER_ADMIN]), async (req, res) => {
